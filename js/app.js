...
function bindUi() {
  ...
  els.readerViewport.addEventListener('touchstart', beginReaderSwipe, { passive: true });
  els.readerViewport.addEventListener('touchend', endReaderSwipe, { passive: true });
...
function beginReaderSwipe(event) {
  if (shouldIgnoreReaderSwipe(event)) return;
...
function endReaderSwipe(event) {
  if (!app.swipe || shouldIgnoreReaderSwipe(event)) {
    app.swipe = null;
    return;
  }
  ...
  const deltaX = point.clientX - app.swipe.x;
  const deltaY = point.clientY - app.swipe.y;
  const elapsed = Date.now() - app.swipe.time;
  app.swipe = null;

  const horizontal = Math.abs(deltaX);
  const vertical = Math.abs(deltaY);
  const isPageSwipe = horizontal >= 58 && horizontal > vertical * 1.35 && elapsed <= 900;
...
function shouldIgnoreReaderSwipe(event) {
  if (!app.currentBook || app.highlightMode) return true;
  if (els.readerOverlay?.classList.contains('annotations-open')) return true;
  if (document.querySelector('.app-modal')) return true;

  const target = event?.target;
  if (!target?.closest) return false;

  return Boolean(target.closest('button, input, textarea, a, .annotation-panel, .reader-toolbar'));
}

function attachEpubSwipe(contents) {
  const doc = contents?.document;
  if (!doc || doc.__librarySwipeReady) return;

  doc.__librarySwipeReady = true;
  doc.addEventListener('touchstart', beginReaderSwipe, { passive: true });
  doc.addEventListener('touchend', endReaderSwipe, { passive: true });
}
...
async function openEpub(url, book) {
  ...
  app.epubRendition.on('rendered', (_section, view) => attachEpubSwipe(view?.contents));
